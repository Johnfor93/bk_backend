...
def study_programs():
    if(request.method == "POST"):
        try:
            ...
            cur.execute("""
                SELECT
                    CONCAT('FAL',TO_CHAR(CURRENT_DATE,'YYMM'),LPAD(CAST(COALESCE(CAST(MAX(RIGHT(study_program_code,4)) AS INT)+1,1) AS VARCHAR),4,'0')) as study_program_code
                FROM
                    m_study_program
                WHERE
                    study_program_code LIKE CONCAT('FAL',TO_CHAR(CURRENT_DATE,'YYMM'),'%')
                """)
            data = cur.fetchone()
